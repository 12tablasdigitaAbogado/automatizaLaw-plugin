...
Listá (Glob) la carpeta `modelos/escritos/`, elegí el modelo `.docx` de alegato y extraé su texto con la skill `docx` (los modelos son Word `.docx`, no uses `Read` directo): tomá su estructura, estilo argumentativo y forma de organizar el mérito de la prueba. Si no hay un modelo para esto, usá la estructura estándar del Paso 4 y avisá. No modifiques el modelo.
